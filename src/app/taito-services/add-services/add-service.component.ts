...
export class AddServiceComponent implements OnInit {
  ...
  constructor(private cd: ChangeDetectorRef, public util: UtilService, public dataShare: DataShareService, private api: APIService, private formBuilder: FormBuilder, private change: ChangeDetectorRef) {
  }

  ngOnInit(): void {
    console.log('Service in ng on nit **********');

  }

  ionViewDidEnter() {
    console.log('Service add will load now **********');
    this.getService();
  }
  ...
  changeValue($event): void {
    console.log($event);
    this.dataShare.serviceValue = $event;
    this.setTempService($event);
    ...
    this.api.getServices().subscribe(data => {
        ...
        if (this.url && this.url.length > 0){
          ...
          if (this.selectedService) {
            this.changeValue(this.selectedService);
